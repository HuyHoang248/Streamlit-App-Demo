...
def app():
    ...
    st.markdown("<h1 style='text-align: center; color: purple;'>Demand Forecasting - By Store </h1>", unsafe_allow_html=True)
    ...
    store = st.selectbox('Choose Store you want to show', (list(np.arange(1,45,1))))
    ...
    if data_file is not None:
        ...
        with col1:
            ...
            st.dataframe(sales_df[sales_df['Store']==str(store)])
                
        with col2:
            st.markdown('Sales Forecasting by Store')
            df_store = output_df[output_df['Store']==store].drop(columns=['Store'])
            ...
            plt.title('Sales Forecasting by Store')
            st.pyplot(fig)
